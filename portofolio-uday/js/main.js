...
//hamburgerToggle.addEventListener('change', toggleMenu);
...
function createScrollLink() {
  ...
  scrollLink.classList.add('read__more');
  scrollLink.textContent = 'Lees Meer >';
  scrollLink.href = '#about';
  scrollLink.addEventListener('click', scrollToAbout);

  const container = document.querySelector('.about__me');
...
function initializeTypedAnimation() {
  ...
  typedAnimation.onAnimationComplete(() => {
    typedAnimation.addReadMoreButton();
  });

  typedAnimation.startAnimation();
}

window.addEventListener('load', initializeTypedAnimation);
